...
\begin{abstract}
This paper presents Autowerkstatt4null, a three-year initiative to empower independent automotive workshops with AI-driven, federated diagnostics. 
...
An oscilloscope is an electrical device with the purpose of projecting a time varying voltage (oscillation) onto a visible plane (scope), 
and with that displaying analytical features of the voltage visibly.
...
such as milage, sensor type, make-and-model of a car to a server’s interface called API. 
...
The gathered data is curretnly stored in a cephfs storage network located at the THGA in Bochum.
...
% 4. Summary
\section{Summary}
The Autowerkstatt 4.0 project has made substantial progress in developing an innovative off-board diagnostics ecosystem for independent automotive workshops. 
...
% 5. Outlook
\section{Outlook}
Based on the aw4null project result, researchers, engineers, and application partners are convinced that the proposed, 
...
First: a clear road map of future monatization of the project and its results has to be laid out in order to provide an environment for sustainable product development. 
...
the progress of which can be followed-up on under github.com/OmnAI- project.
...
All supported by the Federal German Bundesnetzagentur and the GAIA-X Project.
A detailed list of contributions can be obtained from Lukas Jakubczyk at the University for Applied Sciences THGA in Bochum; lukas.jakubczyk@thga.de, PROLAB für Produkt + Produktion, Technische Hochschule Georg Agricola, Herner Str. 45, DE-44787 Bochum.
